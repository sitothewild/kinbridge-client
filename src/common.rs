...
// KinBridge: upstream RustDesk's update check phones home to
// api.rustdesk.com/version/latest. We never want the APK to make network
// calls to rustdesk.com for any reason — updates come through the Play
// Store's normal update path. Permanently disabled.
#[tokio::main(flavor = "current_thread")]
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    // KinBridge: never fall back to admin.rustdesk.com. If the custom
    // rendezvous server isn't configured, there is no upstream admin URL;
    // the user must configure their own. See KinBridge's threat model
    // (Olares-only deploy).
    String::new()
}
