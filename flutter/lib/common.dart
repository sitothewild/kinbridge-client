...
import 'package:kinbridge_support/common/formatter/id_formatter.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:kinbridge_support/desktop/widgets/refresh_wrapper.dart';
...
import 'package:kinbridge_support/utils/platform_channel.dart';
...
import 'package:kinbridge_support/native/win32.dart'
    if (dart.library.html) 'package:kinbridge_support/web/win32.dart';
import 'package:kinbridge_support/native/common.dart'
    if (dart.library.html) 'package:kinbridge_support/web/common.dart';
import 'package:kinbridge_support/utils/http_service.dart' as http;
...
Future<void> windowOnTop(int? id) async {
  ...
  if (id == null) {
    ...
    await kbWinManager.registerActiveWindow(kWindowMainId);
  } else {
    WindowController.fromWindowId(id)
      ..focus()
      ..show();
    kbWinManager.call(WindowType.Main, kWindowEventShow, {"id": id});
  }
}
  ...
  if (type != null && id != null) {
    switch (type) {
      case UriLinkType.remoteDesktop:
        Future.delayed(Duration.zero, () {
          kbWinManager.newRemoteDesktop(id!,
              password: password,
      ...
      case UriLinkType.fileTransfer:
        Future.delayed(Duration.zero, () {
          kbWinManager.newFileTransfer(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.viewCamera:
        Future.delayed(Duration.zero, () {
          kbWinManager.newViewCamera(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.portForward:
        Future.delayed(Duration.zero, () {
          kbWinManager.newPortForward(id!, false,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.rdp:
        Future.delayed(Duration.zero, () {
          kbWinManager.newPortForward(id!, true,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.terminal:
        Future.delayed(Duration.zero, () {
          kbWinManager.newTerminal(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
    }
...
connectMainDesktop(String id,
  ...
  if (isFileTransfer) {
    await kbWinManager.newFileTransfer(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isViewCamera) {
    await kbWinManager.newViewCamera(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTcpTunneling || isRDP) {
    await kbWinManager.newPortForward(id, isRDP,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTerminal) {
    await kbWinManager.newTerminal(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else {
    await kbWinManager.newRemoteDesktop(id,
        password: password,
        isSharedPassword: isSharedPassword,
        forceRelay: forceRelay);
  }
...
connect(BuildContext context, String id,
  ...
  if (isDesktop) {
    if (desktopType == DesktopType.main) {
      ...
      await kbWinManager.call(WindowType.Main, kWindowConnect, {
        'id': id,
        'isFileTransfer': isFileTransfer,
        'isViewCamera': isViewCamera,
        'isTerminal': isTerminal,
...
Future<void> onActiveWindowChanged() async {
  print(
      "[MultiWindowHandler] active window changed: ${kbWinManager.getActiveWindows()}");
  if (kbWinManager.getActiveWindows().isEmpty) {
    // close all sub windows
    try {
      if (isLinux) {
        await Future.wait([
          saveWindowPosition(WindowType.Main),
          kbWinManager.closeAllSubWindows()
        ]);
      } else {
...
Future<List<Rect>> getScreenListWayland() async {
  ...
  if (isMainDesktopWindow) {
    ...
    final screenList = await kbWinManager.call(
